...
namespace Inergy.Indev3.BusinessLogic.Catalogues
{
    ...
    /// </summary>
    public class InergyCountry : GenericEntity, ICountry
    {
        ...
        #region Constructors
        public InergyCountry()
            : this(null)
        {

        }
        public InergyCountry(object connectionManager)
            : base(connectionManager)
        {
            SetEntity(new DBInergyCountry(connectionManager));
            _IdCurrency = ApplicationConstants.INT_NULL_VALUE;
            _IdRegion = ApplicationConstants.INT_NULL_VALUE;
        }
        public InergyCountry(DataRow row, object connectionManager)
