...
namespace Inergy.Indev3.BusinessLogic.Budget
{
    public class FollowUpCompletionBudget : GenericEntity, IFollowUpCompletionBudget
    {
		...
		public int CopyCompletionBudget(int ProjectId, int IdAssociateLM, int IdAssociateNM)
		{
			...
			try
			{
				...
				result = this.GetEntity().ExecuteCustomProcedureWithReturnValue("CopyCompletionBudget", this);
				CommitTransaction();
			}
			catch (Exception ex)
			{
