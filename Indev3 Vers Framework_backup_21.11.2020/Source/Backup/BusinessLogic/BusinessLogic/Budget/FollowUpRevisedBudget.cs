...
namespace Inergy.Indev3.BusinessLogic.Budget
{
    public class FollowUpRevisedBudget : GenericEntity, IFollowUpRevisedBudget
    {
        ...
        public int CopyRevisedBudget(int ProjectId, int IdAssociateLM, int IdAssociateNM)
        {
			...
			try
			{
				...
				result = this.GetEntity().ExecuteCustomProcedureWithReturnValue("CopyRevisedBudget", this);
				CommitTransaction();
			}
			catch (Exception ex)
			{
