...
namespace Inergy.Indev3.ApplicationFramework.Common
{
    public static class DSUtils
    {
        ...
        public static string BuildCSVExport(DataRow[] rows, List<string> columnNames, bool hasHeader)
        {
            ...
            return BuildCSVExport(rows, columnNames, hasHeader, ";"); 
        }

        public static string BuildCSVExport(DataRow[] rows, List<string> columnNames, bool hasHeader, string separator)
        {
            ...
            foreach (DataRow row in rows)
            {
                ...
                for (int i = 0; i < row.ItemArray.Length; i++)
                {
                    ...
                    if (columnNames.Contains(row.Table.Columns[i].ColumnName))
                    {
                       ...
                       //if separator is comma, the fields that contain comma must be surrounded by double quote
                       if (tempString.Contains(",") && separator == ",") tempString = "\"" + tempString + "\"";

                       fileContent.Append(tempString + separator);
                    }
                }
        ...
        public static string BuildCSVExport(DataRow[] rows, List<string> columnNames, bool hasHeader, string separator, bool dateWithTime)
        {
            //StringBuilder storing the data in csv formatting
            StringBuilder fileContent = new StringBuilder();
            int intIterator = ApplicationConstants.INT_NULL_VALUE;
