...
namespace Inergy.Indev3.WebFramework.WebControls
{
    ...
    public class IndYearMonth : CompositeControl, INamingContainer
    {
        ...
        protected IndComboBox cmbYear = new IndComboBox();
        /// <summary>
        /// The month combobox
        /// </summary>
        //protected RadComboBox cmbMonth = new RadComboBox();
        protected IndComboBox cmbMonth = new IndComboBox();
        /// <summary>
        ...
        public object GetValue()
        {
            try
            {
                ...
                if (cmbYear.SelectedItem == null || String.IsNullOrEmpty(cmbYear.SelectedItem.Text) || cmbMonth.SelectedItem == null || String.IsNullOrEmpty(cmbMonth.SelectedItem.Text))
                    return ApplicationConstants.INT_NULL_VALUE;
                //Concatenate the values in the comboboxes
                string month = (cmbMonth.SelectedItem.Text.Length == 1) ? "0" + cmbMonth.SelectedItem.Text : cmbMonth.SelectedItem.Text;
                string yearMonth = cmbYear.SelectedItem.Text + month;
        ...
        private void PopulateControls()
        {
            ...
            cmbYear.Items.Add(new Telerik.Web.UI.RadComboBoxItem(String.Empty));
            for (int yearIterator = _FirstYear; yearIterator <= _LastYear; yearIterator++)
                cmbYear.Items.Add(new Telerik.Web.UI.RadComboBoxItem(yearIterator.ToString()));
            if (yearFromValue == 0)
            {
                cmbYear.SelectedIndex = yearNow - _FirstYear + 1;
            }
            else
            {
                cmbYear.SelectedIndex = yearFromValue - _FirstYear + 1;
            }
            //Set the Month Combobox datasource
            cmbMonth.Items.Add(new Telerik.Web.UI.RadComboBoxItem(String.Empty));
            for (int monthIterator = 1; monthIterator <= 12; monthIterator++)
                cmbMonth.Items.Add(new Telerik.Web.UI.RadComboBoxItem(monthIterator.ToString()));
        }
