...
function CheckIfProjectWasSelectedWithDirtyCheck(name, height, width, sessionExpiredUrl, returnUrl)
{
    var isPageDirty = CheckDirtyBeforeOpeningPopUp();
    if (isPageDirty) {
        if (!confirm("There are unsaved changes on this page. If you click OK you may lose all unsaved changes. Are you sure you want to close this window?")) {
    ...
    if (projectName != null && projectName.includes('Current project')) {
        return 1
    }
    else {
        ShowPopUpWithoutPostBackWithDirtyCheck(name, height, width, sessionExpiredUrl, returnUrl)
    }
}

function ShowPopUpWithoutPostBackWithDirtyCheck(name,height,width,sessionExpiredUrl, returnUrl) 
{
    var isPageDirty = CheckDirtyBeforeOpeningPopUp();
    if (isPageDirty)
    {
    ...
    var hdnReturnUrl = document.getElementById("hdnReturnUrl");
    if (hdnReturnUrl != null)
    {
        if (returnUrl != null) {
            console.log(returnUrl);
            hdnReturnUrl.value = returnUrl;
        }
        else
        {
            hdnReturnUrl.value = "";
    ...
    //var returnValue = window.showModalDialog(name, null, "status:no;center:yes;resizable:no;help:no;dialogHeight:" + height + "px;dialogWidth:" + width + "px");
    radopen(name, "RadWindow1",width,height);
    //If the ok button is pressed
    //if (returnValue == 1)
    //{
    //    //If the user pressed the save button in the pop-up, clear the dirty flag
