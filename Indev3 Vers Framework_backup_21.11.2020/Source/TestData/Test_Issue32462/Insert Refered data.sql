--this script is created especially for testing of issue 32462
-- run first section frnace, then section germany
...
IF NOT EXISTS(SELECT * FROM COUNTRIES WHERE Code='GER')
BEGIN
	DECLARE @IDEURO INT
	DECLARE @RANK INT
	SELECT @IdEuro = Id FROM CURRENCIES WHERE CODE='EUR'
	...
	exec catInsertCountry 'GER', 'GERMANY',NULL, @IdEuro,NULL,@RANK

END

IF NOT EXISTS(SELECT ID FROM INERGY_LOCATIONS WHERE CODE='ILG')
BEGIN
	DECLARE @IDCOUNTRY INT
	SELECT @IDCOUNTRY = ID FROM COUNTRIES WHERE CODE='GER'
	DECLARE @ILRANK INT
	SELECT @ILRANK = MAX(RANK)+1 FROM INERGY_LOCATIONS

	exec catInsertInergyLocation @IDCOUNTRY, 'ILG','INERGY LOC GERMANY', @ILRANK

END

DECLARE @IDINERGYLOCATION INT
SELECT @IDINERGYLOCATION = ID 
FROM INERGY_LOCATIONS 
WHERE CODE='ILG'

IF NOT EXISTS(SELECT ID FROM COST_CENTERS WHERE CODE='A61050' and IdInergyLocation = @IDINERGYLOCATION)
BEGIN
	DECLARE @IDDEPART INT
	SELECT @IDDEPART = ID FROM DEPARTMENTS WHERE NAME='Department1'

	exec catInsertCostCenter @IDINERGYLOCATION, @IDDEPART,1,'A61050', 'COSTCENTER MUNCHEN'
END


DECLARE @IDCOUNTRYGL INT
SELECT @IDCOUNTRYGL = ID FROM COUNTRIES WHERE CODE='GER'

IF NOT EXISTS(SELECT ID FROM GL_ACCOUNTS WHERE ACCOUNT='43A71050' AND IDCOUNTRY=@IDCOUNTRYGL)
BEGIN
	EXEC catInsertGlAccount @IDCOUNTRYGL, '43A71050', 'GERHOURS43A', 6

	EXEC catInsertGlAccount @IDCOUNTRYGL, '43A71056', 'GERSALES43A', 7
	EXEC catInsertGlAccount @IDCOUNTRYGL, '43A71051', 'GERTE43A', 1
	EXEC catInsertGlAccount @IDCOUNTRYGL, '43A71052', 'GERPROTOPARTS43A', 2
	EXEC catInsertGlAccount @IDCOUNTRYGL, '43A71053', 'GERTOOLING43A', 3
	EXEC catInsertGlAccount @IDCOUNTRYGL, '43A71054', 'GERTRIALS43A', 4
	EXEC catInsertGlAccount @IDCOUNTRYGL, '43A71055', 'GEROTHER43A', 5
END
