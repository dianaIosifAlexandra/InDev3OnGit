IF EXISTS (SELECT * FROM dbo.sysobjects WHERE ID = object_id(N'dbo.OLAP_ALLCATEGORIES_DATA') AND OBJECTPROPERTY(id, N'IsView') = 1)
DROP VIEW OLAP_ALLCATEGORIES_DATA
GO

CREATE VIEW OLAP_ALLCATEGORIES_DATA
AS
...
SELECT BID.IdProject,
	...
	BID.Sales,
	BID.CostTypeId,
	CAST(NULL as decimal(18,2)) as Progress, --we do not have progress in Initial Budget
	10 as CategoryId --Select Id FROM OLAP_CATEGORIES where Name = 'Initial Budget'
FROM OLAP_BUDGET_INITIAL_DATA BID
UNION ALL
SELECT BRD.IdProject,
	...
	CAST(11 as int) as CategoryId --Select Id FROM OLAP_CATEGORIES where Name = 'Revised Budget'
FROM OLAP_BUDGET_REVISED_DATA BRD
UNION ALL
...
WHERE dbo.fnGetToCompletionBudgetGeneration(IdProject, 'C') = IdGeneration AND [Percent] IS NOT NULL
GO
