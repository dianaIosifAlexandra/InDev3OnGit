"--Get non empty exchange rates from euro, for 2007
SELECT {[Measures].[Exchange Rate]} ON COLUMNS,
            NON EMPTY CROSSJOIN  ({[PERIODS].[Year].[2007].Children}, {[CUR_Code].[Currency Code].Members}) ON ROWS
FROM Exchange_Rates
WHERE ([CUR_NAME].[EUR-Euro])
"
"select
