"--Get hourly rates greater than 1000 for year 2007
WITH SET [CostCenters] as '{[FCT_CC].[Cost Center Name].Members}'
          SET [Currencies2007] as ' Filter(CROSSJOIN ({[PERIODS].[Total].[2007].Children}, {[CUR_CODE].[Currency Code].Members}), ([Measures].[Hourly Rate]) > 1000)'
SELECT { [Measures].[Hourly Rate]} ON COLUMNS,
           [Currencies2007]  ON  ROWS
FROM Hourly_Rates"
"select
