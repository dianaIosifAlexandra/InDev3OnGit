@echo off
echo Build Setup Deliveries Starting...
...
if "%TARGET_DIR%"=="" SET TARGET_DIR="..\..\Deliveries\%THEDATE%"
...
echo Register VS10 environment variables and paths
if "%VSINSTALLDIR%"=="" call "%VS100COMNTOOLS%\vsvars32.bat" > nul

echo Call the procedure to get the lates version of sources
call update_from_svn.bat
if errorlevel 1 goto err
...
:err
  echo   ******************************************************************************
  echo      Build Setup Deliveries has encountered an error!
  echo   ******************************************************************************
  echo Build Setup Deliveries was NOT completed.
  goto batchfail

:ENDING
  echo Build Setup Deliveries Completed Succesfully
  goto batchsuccess
