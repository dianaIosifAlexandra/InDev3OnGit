<%@ control language="C#" autoeventwireup="true" inherits="UserControls_Profile_Profile, App_Web_ck53ujep" %>
...
<script type="text/javascript">
    //In IndGenericGrid, Role column, the name of role wrap in IE8 and it doesn't in IE7
    //So, in IE7 the grid becomes wider than 538px which is the width of Panel2. But the footer remains
    //in all situation with width=538px.
     //In IE7 Panel2 doesn't extend to right
    if (GetInternetExplorerVersion() == 7) {
        var divPanel = document.getElementById("<%=Panel2.ClientID %>");
        if (divPanel !== null && divPanel.style !== null && divPanel.style["width"] !== null) {
            if (divPanel.style["width"] != "544px") {
                divPanel.style["width"] = "544px";
            }
        }
    }
</script>